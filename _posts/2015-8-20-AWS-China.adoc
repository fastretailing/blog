...
=== Great Firewall
This is another very well-known unique restriction in mainland China. You can find what it is and how it works in somewhere online(Please just google it or just jump to https://en.wikipedia.org/wiki/Great_Firewall[wikipedia]). But in a nuts shell, all inbound/outbound traffics on HTTP(80)/HTTPs(443) are monitored and censored all the time, and would be blocked at DNS level if China government found your web service providing contents which are against China government.
Especially when traffic goes out of mainland China, it is strictly censored and makes traffic roundtrip extremely slow.
...
We are really looking for Lambda support and API Gateway as we will be heavily relying on these 2 services down the road. I hope they will become available even in China region soon.
